...
import ehrax as rx
from ehrax import COLUMN, CodingScheme, CodingSchemesManager, FrozenDict11, FrozenDict1N
...
class TestScopedNames:
    ...
    def suffixes(self) -> MIMICDatasetSchemeSuffixes:
        return MIMICDatasetSchemeSuffixes(gender='bin_gender', ethnicity='ethnicity', dx_discharge='dx_mixed_icd',
                                          obs='obs', icu_inputs='icu_inputs', icu_procedures=None,
                                          hosp_procedures='pr_mixed_icd')

    @pytest.fixture(scope='class', params=[
        '.', ':', 'xxx', '**', '-', '_', '!', '%', '/', 'no justice no peace'
    ])
    ...
    @pytest.fixture(scope='class')
    def variable_scoped_name(self, suffixes: MIMICDatasetSchemeSuffixes, global_infix: tuple[str, ...],
                             name_separator: str, prefix: str) -> ScopedSchemeNames:
        return ScopedSchemeNames(suffixes=suffixes, global_infix=global_infix,
                                 name_separator=name_separator, name_prefix=prefix)

    @pytest.fixture(scope='class')
    def constant_scoped_name(self, suffixes) -> ScopedSchemeNames:
        return ScopedSchemeNames(suffixes=suffixes, global_infix=(), name_separator='_',
                                 name_prefix='SSS')

    def test_scoped_names0(self, constant_scoped_name: ScopedSchemeNames, suffixes: MIMICDatasetSchemeSuffixes):
        assert constant_scoped_name.gender == f'SSS_bin_gender'
        assert constant_scoped_name.ethnicity == f'SSS_ethnicity'
        assert constant_scoped_name.dx_discharge == f'SSS_dx_mixed_icd'
        assert constant_scoped_name.icu_procedures is None

    def test_scoped_names(self, variable_scoped_name: ScopedSchemeNames, suffixes: MIMICDatasetSchemeSuffixes,
    ...
    def test_scoped_names_mapped(self, variable_scoped_name: ScopedSchemeNames, suffixes: MIMICDatasetSchemeSuffixes,
                                 global_infix: tuple[str, ...],
                                 prefix: str,
                                 name_separator: str):
        infix = f"{name_separator.join(global_infix)}{name_separator}" if len(global_infix) > 0 else ''
...
class TestMIMIC3Diagnoses:
    ...
    def updated_manager(self, init_manager: CodingSchemesManager, in_memory_data: Any,
        ...
        return table_resource.register_scheme(init_manager, name=MIXED_SCHEME_NAME,
                                              space_table=table_resource.space(in_memory_data),
                                              icd_version_schemes=FrozenDict11(
                                                  {'9': 'light_icd9'}),
                                              icd_version_selection=None,
                                              target_name=None,
                                              mapping=None)

    @pytest.fixture(scope='class')
    def after_coerce_id2str(self, diagnoses_icd_std: pd.DataFrame,
                            table_resource: MixedICDTableResource_MIMICIII) -> pd.DataFrame:
    ...
    @pytest.fixture(scope='class')
    def mixed_format(self, diagnoses_icd_std: pd.DataFrame, updated_manager: CodingSchemesManager,
                     table_resource: MixedICDTableResource_MIMICIII) -> CodingSchemesManager:
        pipeline = (MixedICDTableResource_MIMICIII._filter_null_codes,
    ...
    def test_coerce_version2str(self, diagnoses_icd_std: pd.DataFrame, ):
    ...
    def test_mixed_format(self, mixed_format: pd.DataFrame, diagnoses_icd_std: pd.DataFrame,
    ...
    def test_pipeline_without_null_code_filtration(self, updated_manager: CodingSchemesManager,
                                                   diagnoses_icd_std: pd.DataFrame):
        bad_pipeline = (lambda df: MixedICDTableResource_MIMICIII._coerce_columns_to_str(df, (COLUMN.code,)),
                        MixedICDTableResource_MIMICIII._strip_icd_codes,
                        MixedICDTableResource_MIMICIII._add_version_column_if_not_exists,
                        MixedICDTableResource_MIMICIII._coerce_version_to_str,
                        MixedICDTableResource_MIMICIII._mixed_code_format(MIXED_SCHEME_NAME, updated_manager))
