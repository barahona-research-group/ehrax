...
class SampleSubjects(TVxTransformation):
    ...
    def apply(cls, tvx_ehr: TVxEHR, schemes_context: CodingSchemesManager, report: TVxReport) -> tuple[
        ...
        report = report.add(table='static', column='index', before=n1, after=n2, value_type='count',
                            transformation=cls,
                            operation='sample')
        dataset = eqx.tree_at(lambda x: x.columns.static, tvx_ehr.dataset, static)
...
class TrainingSplitGroups(TVxTransformation):
    ...
    def sync_dataset(cls, dataset: Dataset, subject_ids: tuple[str, ...]) -> Dataset:
        ...
        dataset = eqx.tree_at(lambda x: x.columns.static, dataset, static)
    ...
    @classmethod
    def split_dataset(cls, tvx_ehr: TVxEHR, n_groups: int, seed: int = 0,
                      split_balance: Optional[SplitLiteral] = None) -> \
            tuple[TVxEHR, ...]:
...
class ZScoreScaler(CodedValueScaler):
    ...
    def __init__(self, config: ScalerConfig, mean: pd.Series = pd.Series(), std: pd.Series = pd.Series(),
...
class MaxScaler(CodedValueScaler):
    ...
    def __init__(self, config: ScalerConfig, max_val: pd.Series = pd.Series(),
                 table_name: Optional[str] = None,
                 code_column: Optional[str] = None, value_column: Optional[str] = None
                 ):
        super().__init__(config=config, table_name=table_name, code_column=code_column, value_column=value_column)
...
class AdaptiveScaler(CodedValueScaler):
    ...
    def __init__(self, config: ScalerConfig, max_val: pd.Series = pd.Series(),
                 min_val: pd.Series = pd.Series(), mean: pd.Series = pd.Series(), std: pd.Series = pd.Series(),
                 table_name: Optional[str] = None, code_column: Optional[str] = None,
...
class IQROutlierRemover(CodedValueProcessor):
    ...
    def __init__(self, config: IQROutlierRemoverConfig,
...
class ObsIQROutlierRemover(TrainableTransformation):
    ...
    def apply(cls, tvx_ehr: TVxEHR, schemes_context: CodingSchemesManager, report: TVxReport) -> tuple[
        ...
        remover = IQROutlierRemover(config=config).fit(tvx_ehr.dataset, cls.get_admission_ids(tvx_ehr),
                                                       table_name='obs',
                                                       code_column=tvx_ehr.dataset.config.columns.obs.code_alias,
                                                       value_column=tvx_ehr.dataset.config.columns.obs.value_alias)
        tvx_ehr = eqx.tree_at(lambda x: x.numerical_processors.outlier_removers.obs, tvx_ehr, remover,
...
class ObsAdaptiveScaler(TrainableTransformation):
    ...
    def apply(cls, tvx_ehr: TVxEHR, schemes_context: CodingSchemesManager, report: TVxReport) -> tuple[
        ...
        tvx_ehr = eqx.tree_at(lambda x: x.numerical_processors.scalers.obs, tvx_ehr, scaler,
                              is_leaf=lambda x: x is None)
        report = report.add(
            table='obs', column=None, value_type='type',
            transformation=cls,
        ...
        dtype1 = tvx_ehr.dataset.columns.obs[value_column].dtype
        tvx_ehr = eqx.tree_at(lambda x: x.dataset, tvx_ehr, scaler(tvx_ehr.dataset))
        dtype2 = tvx_ehr.dataset.columns.obs[value_column].dtype
...
class InputScaler(TrainableTransformation):
    ...
    def apply(cls, tvx_ehr: TVxEHR, schemes_context: CodingSchemesManager, report: TVxReport) -> tuple[
        ...
        tvx_ehr = eqx.tree_at(lambda x: x.numerical_processors.scalers.icu_inputs, tvx_ehr, scaler,
                              is_leaf=lambda x: x is None)
        report = report.add(
        ...
        dtype1 = tvx_ehr.dataset.columns.icu_inputs[value_column].dtype
        tvx_ehr = eqx.tree_at(lambda x: x.dataset, tvx_ehr, scaler(tvx_ehr.dataset))
        dtype2 = tvx_ehr.dataset.columns.icu_inputs[value_column].dtype
...
class TVxConcepts(TVxTransformation):
    ...
    def _static_info(cls, tvx_ehr: TVxEHR, schemes_context: CodingSchemesManager, report: TVxReport) -> tuple[
        ...
        if tvx_scheme_proxy.ethnicity is not None and config.ethnicity:
            ...
            ethnicity_dict = {subject_id: ethnicity_m.map_codeset({c}) for subject_id, c in
    ...
    @staticmethod
    def _dx_discharge_history(tvx_ehr: TVxEHR, schemes_context: CodingSchemesManager,
                              dx_discharge: dict[str, CodesVector]) -> dict[str, CodesVector]:
    ...
    def _hosp_procedures(tvx_ehr: TVxEHR, schemes_context: CodingSchemesManager) -> dict[str, InpatientInput]:
        return TVxConcepts._procedures(schemes_context,
    ...
    @classmethod
    def _interventions(cls, tvx_ehr: TVxEHR, schemes_context: CodingSchemesManager, report: TVxReport) -> tuple[
        dict[str, InpatientInterventions], TVxReport]:
