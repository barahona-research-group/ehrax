...
def scheme(name: str, codes: list[str]) -> rx.CodingScheme:
    return rx.CodingScheme(name=name, codes=tuple(sorted(codes)),
...
def _static_info(ethnicity: rx.CodesVector, gender: rx.CodesVector) -> rx.StaticInfo:
    return rx.StaticInfo(ethnicity=ethnicity, gender=gender,
...
def _dataset_tables(dataset_tables_config: rx.DatasetTablesConfig,
    ...
    return rx.DatasetTables(static=subjects_df,
...
SCHEMES: dict[str, rx.CodingScheme] = dict(
    ...
    obs=rx.NumericScheme(name='observation11',
...
TARGET_SCHEMES_MAPS['icu_inputs'] = rx.ReducedCodeMapN1.from_data(SCHEMES['icu_inputs'].name,
...
DATASET_SCHEME_CONF = rx.DatasetSchemeConfig(ethnicity=SCHEMES['ethnicity'].name,
...
def leading_observables_extractor(observation_scheme: rx.NumericScheme,
                                  ...
                                  code_index: int = BINARY_OBSERVATION_CODE_INDEX) -> rx.LeadingObservableExtractor:
    config = rx.LeadingObservableExtractorConfig(observable_code=observation_scheme.codes[code_index],
