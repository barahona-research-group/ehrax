...
class CCSMapOps:
    ...
    def register_mappings(cls, manager: CodingSchemesManager, ccs_scheme: str,
        ...
        manager = manager.add_map(CodeMap(source_name=icd9_scheme.name,
                                          target_name=ccs_scheme.name, data=FrozenDict1N(dict(icd92ccs))))
        manager = manager.add_map(CodeMap(source_name=ccs_scheme.name,
                                          target_name=icd9_scheme.name, data=FrozenDict1N(dict(ccs2icd9))))
        return manager
...
class CCSHierarchicalScheme(HierarchicalScheme):
    ...
    def create_scheme(cls, manager: CodingSchemesManager) -> CodingSchemesManager:
        ...
        ch2pt = HierarchicalScheme.reverse_connection(FrozenDict1N(cls.ops.parent_child_mappings(df)))
        desc = FrozenDict11(cls.ops.desc_mappings(df))
...
class DxCCS(CCSHierarchicalScheme):
    ...
    def __init__(self, name: str, codes: tuple[str, ...], desc: Optional[FrozenDict11[str]] = None,
                 dag_codes: Optional[Iterable[str]] = None, dag_desc: Optional[Iterable[str]] = None,
                 code2dag: Optional[FrozenDict11[str]] = None, ch2pt: Optional[FrozenDict1N] = None,
                 ICD9_SCHEME_NAME: str = 'dx_icd9', SCHEME_NAME: str = 'dx_ccs'):
        super().__init__(name=name, codes=codes, desc=desc, dag_codes=dag_codes, ch2pt=ch2pt, dag_desc=dag_desc,
                         code2dag=code2dag)
...
class PrCCS(CCSHierarchicalScheme):
    ...
    def __init__(self, name: str, codes: tuple[str, ...], desc: Optional[FrozenDict11[str]] = None,
                 dag_codes: Optional[Iterable[str]] = None, dag_desc: Optional[Iterable[str]] = None,
                 code2dag: Optional[FrozenDict11[str]] = None, ch2pt: Optional[FrozenDict1N] = None,
                 ICD9_SCHEME_NAME: str = 'pr_icd9', SCHEME_NAME: str = 'pr_ccs'):
        super().__init__(name=name, codes=codes, desc=desc, dag_codes=dag_codes, ch2pt=ch2pt, dag_desc=dag_desc,
                         code2dag=code2dag)
        self.ICD9_SCHEME_NAME = ICD9_SCHEME_NAME
        self.SCHEME_NAME = SCHEME_NAME
...
class FlatCCSScheme(CodingScheme):
    ...
    def create_scheme(cls, manager: CodingSchemesManager) -> CodingSchemesManager:
        ...
        return manager.add_scheme(cls(name=cls.SCHEME_NAME,
                                      codes=codes,
                                      desc=FrozenDict11(dict(zip(cols['code'], cols['desc'])))))
...
class DxFlatCCS(FlatCCSScheme):
    ...
    def __init__(self, name: str, codes: tuple[str, ...], desc: Optional[FrozenDict11[str]] = None,
                 ICD9_SCHEME_NAME: str = 'dx_icd9', SCHEME_NAME: str = 'dx_flat_ccs'):
...
class PrFlatCCS(FlatCCSScheme):
    ...
    def __init__(self, name: str, codes: tuple[str, ...], desc: Optional[FrozenDict11[str]] = None,
                 ICD9_SCHEME_NAME: str = 'pr_icd9', SCHEME_NAME: str = 'pr_flat_ccs'):
        super().__init__(name=name, codes=codes, desc=desc)
        self.ICD9_SCHEME_NAME = ICD9_SCHEME_NAME
        self.SCHEME_NAME = SCHEME_NAME
